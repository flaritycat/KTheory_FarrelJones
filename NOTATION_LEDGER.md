...
| Magnus hierarchy | hierarchy of one-relator groups using HNN splittings with lower-complexity one-relator vertex group and Magnus-subgroup edge groups | source-level active | FJ21 | Bridge description checked in Linton's `Hyperbolic one-relator groups`; detailed theorem formulation deferred to FJ22. |
| one-relator hierarchy | modern hierarchy vocabulary following Linton's refinement of the Magnus--Moldavanskii hierarchy | source-level active | FJ21 | Chosen as the `T-001` structure toolkit; exact theorem statements to be selected in FJ22. |
| \(\mathbb Z\)-stable HNN extension | source-level Linton hierarchy term | source-level flag | FJ21 | Located in the selected primary hierarchy source; exact definition deferred to FJ22. |
| \(\mathbb Z\)-stable hierarchy | source-level Linton hierarchy term | source-level flag | FJ21 | Located in the selected primary hierarchy source; exact definition deferred to FJ22. |
