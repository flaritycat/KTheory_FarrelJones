...
| source-version collapse | project warning for treating coefficient K-theory, full \(\mathcal{FJ}\), `FJCw`, `FICwF`, or CAT(0)-route statements as interchangeable | active warning term | FJ45 | Avoid before `FJ02` or a source-conventions module supplies a precise comparison. |
| source-convention interruption | deliberate pause in a target thread to complete a foundations/source-comparison module before further proof-sensitive route use | active project term | FJ46 | `FJ46` applies this to `RB-005`, selecting `FJ02` before more finite-index formulation work. |
