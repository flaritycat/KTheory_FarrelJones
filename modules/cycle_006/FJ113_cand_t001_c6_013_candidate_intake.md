# Module FJ113. CAND-T001-C6-013 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-013-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ112`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-013-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-013`, \(G_{C6,013}=\langle a,b\mid a b^{15} a^{-1} b^{-29} a b^{30} a^{-1} b^{-15}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ113`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ113` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ113_cand_t001_c6_013_candidate_intake.md` |
...
\qquad
r=a b^{15} a^{-1} b^{-29} a b^{30} a^{-1} b^{-15}.
\]
...
\[
\bar\chi\colon G_{C6,013}\to\mathbb Z.
\]
...
`CAND-T001-C6-013` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` and `FJ101`--`FJ113` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,013}\to\mathbb Z\) is recorded;
...
`OBL-C6-016`: no later module may promote, route, or subtract
`CAND-T001-C6-013` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ113`. Do not execute queue row `014` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-013-2026-001` for `FJ113` only;
- candidate row `CAND-T001-C6-013`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ113` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,013}\to\mathbb Z\);
- proof obligation `OBL-C6-016`.

No established result number is produced.

## Open questions generated

- `OQ-139`: Which accepted payload, if any, should act on
  `CAND-T001-C6-013` after the `FJ113` candidate-intake audit?
