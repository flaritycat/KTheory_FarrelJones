# Module FJ143. CAND-T001-C6-043 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-043-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ142`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-043-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-043`, \(G_{C6,043}=\langle a,b\mid a b^{45} a^{-1} b^{-89} a b^{90} a^{-1} b^{-45}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ143`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ143` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ143_cand_t001_c6_043_candidate_intake.md` |
...
\qquad
r=a b^{45} a^{-1} b^{-89} a b^{90} a^{-1} b^{-45}.
\]
...
\[
\bar\chi\colon G_{C6,043}\to\mathbb Z.
\]
...
`CAND-T001-C6-043` is torsion-free at first-pass candidate-ledger level.
...
`CAND-T001-C6-043` is candidate-admissible at first-pass intake level and
route-unresolved.
...
- first-pass torsion-free candidate status using the existing repository
  source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,043}\to\mathbb Z\) is recorded;
- no known route or prior-art blocker recorded.

## Proof obligation created

`OBL-C6-046`: do not promote, route, or treat as residual-subtractive
`CAND-T001-C6-043` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ143`. Do not execute queue row `044` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-043-2026-001` for `FJ143` only;
- candidate row `CAND-T001-C6-043`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ143` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,043}\to\mathbb Z\);
- proof obligation `OBL-C6-046`.

No established result number is produced.

## Open questions generated

- `OQ-169`: Which accepted payload, if any, should act on
  `CAND-T001-C6-043` after the `FJ143` candidate-intake audit?
