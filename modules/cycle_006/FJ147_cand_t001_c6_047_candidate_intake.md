# Module FJ147. CAND-T001-C6-047 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-047-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ146`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-047-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-047`, \(G_{C6,047}=\langle a,b\mid a b^{49} a^{-1} b^{-97} a b^{98} a^{-1} b^{-49}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ147`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ147` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ147_cand_t001_c6_047_candidate_intake.md` |
...
\qquad
r=a b^{49} a^{-1} b^{-97} a b^{98} a^{-1} b^{-49}.
\]
...
\[
\bar\chi\colon G_{C6,047}\to\mathbb Z.
\]
...
`CAND-T001-C6-047` is torsion-free at first-pass candidate-ledger level.
...
`CAND-T001-C6-047` is candidate-admissible at first-pass intake level and
route-unresolved.
...
- first-pass torsion-free candidate status using the existing repository
  source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,047}\to\mathbb Z\) is recorded;
- no known route or prior-art blocker recorded.

## Proof obligation created

`OBL-C6-050`: do not promote, route, or treat as residual-subtractive
`CAND-T001-C6-047` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ147`. Do not execute queue row `048` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-047-2026-001` for `FJ147` only;
- candidate row `CAND-T001-C6-047`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ147` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,047}\to\mathbb Z\);
- proof obligation `OBL-C6-050`.

No established result number is produced.

## Open questions generated

- `OQ-173`: Which accepted payload, if any, should act on
  `CAND-T001-C6-047` after the `FJ147` candidate-intake audit?
