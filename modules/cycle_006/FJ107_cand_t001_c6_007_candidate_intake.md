# Module FJ107. CAND-T001-C6-007 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-007-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ106`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-007-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-007`, \(G_{C6,007}=\langle a,b\mid a b^9 a^{-1} b^{-17} a b^{18} a^{-1} b^{-9}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ107`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ107` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ107_cand_t001_c6_007_candidate_intake.md` |
...
\qquad
r=a b^9 a^{-1} b^{-17} a b^{18} a^{-1} b^{-9}.
\]
...
\[
\bar\chi\colon G_{C6,007}\to\mathbb Z.
\]
...
`CAND-T001-C6-007` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91`--`FJ106` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,007}\to\mathbb Z\) is recorded;
...
`OBL-C6-010`: no later module may promote, route, or subtract
`CAND-T001-C6-007` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ107`. Do not execute queue row `008` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-007-2026-001` for `FJ107` only;
- candidate row `CAND-T001-C6-007`;
- first-pass non-proper-power check by exponent-sum vector \((0,1)\);
- first-pass torsion-free candidate status using the existing `FJ91`--`FJ106`
  source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,007}\to\mathbb Z\);
- proof obligation `OBL-C6-010`.

No established result number is produced.

## Open questions generated

- `OQ-133`: Which accepted payload, if any, should act on
  `CAND-T001-C6-007` after the `FJ107` candidate-intake audit?
