# Module FJ127. CAND-T001-C6-027 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-027-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ126`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-027-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-027`, \(G_{C6,027}=\langle a,b\mid a b^{29} a^{-1} b^{-57} a b^{58} a^{-1} b^{-29}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ127`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ127` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ127_cand_t001_c6_027_candidate_intake.md` |
...
\qquad
r=a b^{29} a^{-1} b^{-57} a b^{58} a^{-1} b^{-29}.
\]
...
\[
\bar\chi\colon G_{C6,027}\to\mathbb Z.
\]
...
`CAND-T001-C6-027` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` and `FJ101`--`FJ127` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,027}\to\mathbb Z\) is recorded;
...
`OBL-C6-030`: no later module may promote, route, or subtract
`CAND-T001-C6-027` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ127`. Do not execute queue row `028` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-027-2026-001` for `FJ127` only;
- candidate row `CAND-T001-C6-027`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ127` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,027}\to\mathbb Z\);
- proof obligation `OBL-C6-030`.

No established result number is produced.

## Open questions generated

- `OQ-153`: Which accepted payload, if any, should act on
  `CAND-T001-C6-027` after the `FJ127` candidate-intake audit?
