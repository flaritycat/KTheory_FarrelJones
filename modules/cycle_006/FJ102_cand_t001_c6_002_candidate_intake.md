# Module FJ102. CAND-T001-C6-002 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-002-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ101`.
...
`CAND-T001-C6-001`, does not compute Brown/BNS data, and does not audit
broad prior art.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-002-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-002`, \(G_{C6,002}=\langle a,b\mid a b^4 a^{-1} b^{-7} a b^8 a^{-1} b^{-4}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91`, `FJ101`, and `FJ102`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ102` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ102_cand_t001_c6_002_candidate_intake.md` |
...
\[
\bar\chi\colon G_{C6,002}\to\mathbb Z.
\]
...
`CAND-T001-C6-002` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` and `FJ101` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,002}\to\mathbb Z\) is recorded;
...
`OBL-C6-005`: no later module may promote, route, or subtract
`CAND-T001-C6-002` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ102`. Do not execute queue row `003` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-002-2026-001` for `FJ102` only;
- candidate row `CAND-T001-C6-002`;
- first-pass non-proper-power check by exponent-sum vector \((0,1)\);
- first-pass torsion-free candidate status using the existing `FJ91` /
  `FJ101` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,002}\to\mathbb Z\);
- proof obligation `OBL-C6-005`.

No established result number is produced.

## Open questions generated

- `OQ-128`: Which accepted payload, if any, should act on
  `CAND-T001-C6-002` after the `FJ102` candidate-intake audit?
