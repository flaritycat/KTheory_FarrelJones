# Module FJ104. CAND-T001-C6-004 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-004-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ103`.
It supplies a concrete two-generator one-relator candidate and asks for a
bounded intake audit only.

This module consumes exactly that row. It does not act on
`CAND-T001-C6-001`, `CAND-T001-C6-002`, or `CAND-T001-C6-003`, does not
compute Brown/BNS data, and does not audit broad prior art.

## Payload intake record

| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-004-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-004`, \(G_{C6,004}=\langle a,b\mid a b^6 a^{-1} b^{-11} a b^{12} a^{-1} b^{-6}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ104`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ104` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ104_cand_t001_c6_004_candidate_intake.md` |
...
\qquad
r=a b^6 a^{-1} b^{-11} a b^{12} a^{-1} b^{-6}.
\]
...
\[
\bar\chi\colon G_{C6,004}\to\mathbb Z.
\]
...
`CAND-T001-C6-004` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91`--`FJ103` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,004}\to\mathbb Z\) is recorded;
...
`OBL-C6-007`: no later module may promote, route, or subtract
`CAND-T001-C6-004` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ104`. Do not execute queue row `005` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-004-2026-001` for `FJ104` only;
- candidate row `CAND-T001-C6-004`;
- first-pass non-proper-power check by exponent-sum vector \((0,1)\);
- first-pass torsion-free candidate status using the existing `FJ91`--`FJ103`
  source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,004}\to\mathbb Z\);
- proof obligation `OBL-C6-007`.

No established result number is produced.

## Open questions generated

- `OQ-130`: Which accepted payload, if any, should act on
  `CAND-T001-C6-004` after the `FJ104` candidate-intake audit?
