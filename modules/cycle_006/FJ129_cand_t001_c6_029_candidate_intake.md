# Module FJ129. CAND-T001-C6-029 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-029-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ128`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-029-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-029`, \(G_{C6,029}=\langle a,b\mid a b^{31} a^{-1} b^{-61} a b^{62} a^{-1} b^{-31}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ129`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ129` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ129_cand_t001_c6_029_candidate_intake.md` |
...
\qquad
r=a b^{31} a^{-1} b^{-61} a b^{62} a^{-1} b^{-31}.
\]
...
\[
\bar\chi\colon G_{C6,029}\to\mathbb Z.
\]
...
`CAND-T001-C6-029` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` and `FJ101`--`FJ129` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,029}\to\mathbb Z\) is recorded;
...
`OBL-C6-032`: no later module may promote, route, or subtract
`CAND-T001-C6-029` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ129`. Do not execute queue row `030` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-029-2026-001` for `FJ129` only;
- candidate row `CAND-T001-C6-029`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ129` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,029}\to\mathbb Z\);
- proof obligation `OBL-C6-032`.

No established result number is produced.

## Open questions generated

- `OQ-155`: Which accepted payload, if any, should act on
  `CAND-T001-C6-029` after the `FJ129` candidate-intake audit?
