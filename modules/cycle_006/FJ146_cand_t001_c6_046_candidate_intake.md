# Module FJ146. CAND-T001-C6-046 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-046-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ145`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-046-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-046`, \(G_{C6,046}=\langle a,b\mid a b^{48} a^{-1} b^{-95} a b^{96} a^{-1} b^{-48}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ146`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ146` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ146_cand_t001_c6_046_candidate_intake.md` |
...
\qquad
r=a b^{48} a^{-1} b^{-95} a b^{96} a^{-1} b^{-48}.
\]
...
\[
\bar\chi\colon G_{C6,046}\to\mathbb Z.
\]
...
`CAND-T001-C6-046` is torsion-free at first-pass candidate-ledger level.
...
`CAND-T001-C6-046` is candidate-admissible at first-pass intake level and
route-unresolved.
...
- first-pass torsion-free candidate status using the existing repository
  source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,046}\to\mathbb Z\) is recorded;
- no known route or prior-art blocker recorded.

## Proof obligation created

`OBL-C6-049`: do not promote, route, or treat as residual-subtractive
`CAND-T001-C6-046` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ146`. Do not execute queue row `047` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-046-2026-001` for `FJ146` only;
- candidate row `CAND-T001-C6-046`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ146` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,046}\to\mathbb Z\);
- proof obligation `OBL-C6-049`.

No established result number is produced.

## Open questions generated

- `OQ-172`: Which accepted payload, if any, should act on
  `CAND-T001-C6-046` after the `FJ146` candidate-intake audit?
