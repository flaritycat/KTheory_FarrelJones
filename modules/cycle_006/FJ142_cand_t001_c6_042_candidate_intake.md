# Module FJ142. CAND-T001-C6-042 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-042-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ141`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-042-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-042`, \(G_{C6,042}=\langle a,b\mid a b^{44} a^{-1} b^{-87} a b^{88} a^{-1} b^{-44}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ142`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ142` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ142_cand_t001_c6_042_candidate_intake.md` |
...
\qquad
r=a b^{44} a^{-1} b^{-87} a b^{88} a^{-1} b^{-44}.
\]
...
\[
\bar\chi\colon G_{C6,042}\to\mathbb Z.
\]
...
`CAND-T001-C6-042` is torsion-free at first-pass candidate-ledger level.
...
`CAND-T001-C6-042` is candidate-admissible at first-pass intake level and
route-unresolved.
...
- first-pass torsion-free candidate status using the existing repository
  source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,042}\to\mathbb Z\) is recorded;
- no known route or prior-art blocker recorded.

## Proof obligation created

`OBL-C6-045`: do not promote, route, or treat as residual-subtractive
`CAND-T001-C6-042` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ142`. Do not execute queue row `043` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-042-2026-001` for `FJ142` only;
- candidate row `CAND-T001-C6-042`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ142` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,042}\to\mathbb Z\);
- proof obligation `OBL-C6-045`.

No established result number is produced.

## Open questions generated

- `OQ-168`: Which accepted payload, if any, should act on
  `CAND-T001-C6-042` after the `FJ142` candidate-intake audit?
