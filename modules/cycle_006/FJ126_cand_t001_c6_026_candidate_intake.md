# Module FJ126. CAND-T001-C6-026 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-026-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ125`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-026-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-026`, \(G_{C6,026}=\langle a,b\mid a b^{28} a^{-1} b^{-55} a b^{56} a^{-1} b^{-28}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ126`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ126` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ126_cand_t001_c6_026_candidate_intake.md` |
...
\qquad
r=a b^{28} a^{-1} b^{-55} a b^{56} a^{-1} b^{-28}.
\]
...
\[
\bar\chi\colon G_{C6,026}\to\mathbb Z.
\]
...
`CAND-T001-C6-026` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` and `FJ101`--`FJ126` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,026}\to\mathbb Z\) is recorded;
...
`OBL-C6-029`: no later module may promote, route, or subtract
`CAND-T001-C6-026` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ126`. Do not execute queue row `027` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-026-2026-001` for `FJ126` only;
- candidate row `CAND-T001-C6-026`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ126` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,026}\to\mathbb Z\);
- proof obligation `OBL-C6-029`.

No established result number is produced.

## Open questions generated

- `OQ-152`: Which accepted payload, if any, should act on
  `CAND-T001-C6-026` after the `FJ126` candidate-intake audit?
