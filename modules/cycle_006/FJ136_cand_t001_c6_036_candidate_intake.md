# Module FJ136. CAND-T001-C6-036 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-036-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ135`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-036-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-036`, \(G_{C6,036}=\langle a,b\mid a b^{38} a^{-1} b^{-75} a b^{76} a^{-1} b^{-38}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ136`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ136` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ136_cand_t001_c6_036_candidate_intake.md` |
...
\qquad
r=a b^{38} a^{-1} b^{-75} a b^{76} a^{-1} b^{-38}.
\]
...
\[
\bar\chi\colon G_{C6,036}\to\mathbb Z.
\]
...
`CAND-T001-C6-036` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` and `FJ101`--`FJ136` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,036}\to\mathbb Z\) is recorded;
...
`OBL-C6-039`: no later module may promote, route, or subtract
`CAND-T001-C6-036` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ136`. Do not execute queue row `037` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-036-2026-001` for `FJ136` only;
- candidate row `CAND-T001-C6-036`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ136` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,036}\to\mathbb Z\);
- proof obligation `OBL-C6-039`.

No established result number is produced.

## Open questions generated

- `OQ-162`: Which accepted payload, if any, should act on
  `CAND-T001-C6-036` after the `FJ136` candidate-intake audit?
