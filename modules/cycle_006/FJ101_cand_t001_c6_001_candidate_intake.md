# Module FJ101. CAND-T001-C6-001 Candidate Intake
...
Numbered module / T-001 candidate intake / payload-gated cycle-006 exit.

## Problem

The project is in `C6-PAUSE-001`, with `OQ-126` open and `OBL-C6-003`
blocking `FJ101` until a concrete accepted payload is recorded. The callback
queue row `PAY-T001-CAND-C6-001-2026-001` supplies a concrete two-generator
one-relator candidate and asks for a bounded intake audit only.

## Payload intake record

| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-001-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-001`, \(G_{C6,001}=\langle a,b\mid a b^3 a^{-1} b^{-5} a b^6 a^{-1} b^{-3}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ101` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ101_cand_t001_c6_001_candidate_intake.md` |
...
\[
\bar\chi\colon G_{C6,001}\to\mathbb Z.
\]
...
`CAND-T001-C6-001` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,001}\to\mathbb Z\) is recorded;
...
`OBL-C6-004`: no later module may promote, route, or subtract
`CAND-T001-C6-001` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ101`. Do not execute queue row `002` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-001-2026-001` for `FJ101` only;
- candidate row `CAND-T001-C6-001`;
- first-pass non-proper-power check by exponent-sum vector \((0,1)\);
- first-pass torsion-free candidate status using the existing `FJ91` source
  status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,001}\to\mathbb Z\);
- proof obligation `OBL-C6-004`.

No established result number is produced.

## Open questions generated

- `OQ-127`: Which accepted payload, if any, should act on
  `CAND-T001-C6-001` after the `FJ101` candidate-intake audit?
