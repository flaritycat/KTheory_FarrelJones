# Module FJ128. CAND-T001-C6-028 Candidate Intake
...
The callback queue row `PAY-T001-CAND-C6-028-2026-001` is the first row in
`ledgers/payload_execution_queue.md` marked `Ready for intake` after `FJ127`.
...
| Field | Entry |
| --- | --- |
| Payload ID | `PAY-T001-CAND-C6-028-2026-001` |
...
| Candidate, source, bridge, computation, or blocker | Candidate `CAND-T001-C6-028`, \(G_{C6,028}=\langle a,b\mid a b^{30} a^{-1} b^{-59} a b^{60} a^{-1} b^{-30}\rangle\), with \(\chi(a)=1,\chi(b)=0\). |
...
| Source-status label | Internal candidate-generation payload. Torsion-free status uses only the existing repository row: Putman's notes are an active reference for `FJ91` and `FJ101`--`FJ128`; first-pass theorem text checked for the non-proper-power torsion-free corollary. No browsing and no new source check. |
...
| Accepted? | Yes, for `FJ128` candidate-intake audit only. |
| Follow-up module if accepted | `modules/cycle_006/FJ128_cand_t001_c6_028_candidate_intake.md` |
...
\qquad
r=a b^{30} a^{-1} b^{-59} a b^{60} a^{-1} b^{-30}.
\]
...
\[
\bar\chi\colon G_{C6,028}\to\mathbb Z.
\]
...
`CAND-T001-C6-028` is torsion-free at first-pass candidate-ledger level.
...
- the torsion-free status is supported at the same first-pass level as
  `FJ91` and `FJ101`--`FJ128` by the existing Putman source-status row;
- the epimorphism \(\bar\chi\colon G_{C6,028}\to\mathbb Z\) is recorded;
...
`OBL-C6-031`: no later module may promote, route, or subtract
`CAND-T001-C6-028` unless a new accepted payload supplies one of:
...
Brown/BNS data in `FJ128`. Do not execute queue row `029` in this run.
...
- accepted intake of `PAY-T001-CAND-C6-028-2026-001` for `FJ128` only;
- candidate row `CAND-T001-C6-028`;
...
- first-pass torsion-free candidate status using the existing `FJ91` and
  `FJ101`--`FJ128` source status for Putman;
- epimorphism \(\bar\chi\colon G_{C6,028}\to\mathbb Z\);
- proof obligation `OBL-C6-031`.

No established result number is produced.

## Open questions generated

- `OQ-154`: Which accepted payload, if any, should act on
  `CAND-T001-C6-028` after the `FJ128` candidate-intake audit?
