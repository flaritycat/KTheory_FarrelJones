...
- Wegner, C. (2012). The K-theoretic Farrell-Jones conjecture for CAT(0)-groups. *Proceedings of the American Mathematical Society, 140*(3), 779--793. https://doi.org/10.1090/S0002-9939-2011-11150-X. Status: `active reference for FJ11, FJ14, FJ15, and FJ17; partially read overall`. Notes: used in `FJ11` for full K-theoretic Farrell--Jones with coefficients for finite-dimensional CAT(0)-groups; `FJ14` checked the introduction, main theorem, strong-transfer-reducibility route, and CAT(0)-group verification theorem; `FJ15` checked the obstruction-category review, control conditions, functoriality condition for control-space maps, and gain-control motivation. `FJ17` checked strong homotopy actions, strong transfer reducibility, the proof-outline transfer diagram, the transfer functor, and the identity \(\operatorname{pr}_*\circ\operatorname{trans}_*=\operatorname{diag}_*\). arXiv version: https://arxiv.org/abs/1012.3349.
...
- Linton, M. (2024). *One-relator hierarchies* (arXiv:2202.11324v3). arXiv. https://arxiv.org/abs/2202.11324. Status: `active reference for FJ22; partially read overall`. Notes: `FJ21` checked the arXiv abstract for its refinement of the Magnus--Moldavanskii hierarchy and its \(\mathbb Z\)-stable HNN/hierarchy vocabulary. `FJ22` checked the arXiv v3 source for the one-relator complex and Magnus subgraph setup, the theorem labeled `new_hierarchy`, the definitions of one-relator tower, one-relator splitting, hierarchy length, \(\mathbb Z\)-stable hierarchy, quasi-convex hierarchy, acylindrical hierarchy, and the main equivalence theorem. Published-version metadata should be checked before relying on final theorem numbering.
