...
Open inside project; generated by `FJ62` and selected for `FJ63`.

### Context

`FJ62` records `OBL-T001-009`: `FJ63` must select exactly one
data-acquisition packet or record failure to do so. The packet must change a
project object and must not be a broad source survey.
