...
their exact source lanes. Continue with `FJ48` before promoting virtually
compact special cases through finite-index passage.
...
subtraction until `FJ48` checks whether the finite-index subgroup input is
`FJCw`, plain coefficient K-theory, full \(\mathcal{FJ}\), or something else.
...
`FJCw` bridge supplied by `FJ47`. Continue with `FJ48`, `RB-005` application
audit.
...
Resolved by `FJ46`; the `FJ02` source-convention interruption and `FJ47`
finite-index bridge selection are now complete. Continue with
`OQ-069`/`FJ48`.
...
Resolved by `FJ47`. Continue with `OQ-069`/`FJ48`: audit which `RB-005`
cases actually have source-verified `FJCw` finite-index subgroup input.

## OQ-069. Which RB-005 cases have FJCw finite-index subgroup input?

### Status

Open inside project; generated by `FJ47` and selected for `FJ48`.
...
`ledgers/inheritance_properties.md`, `ledgers/source_status.md`, and any
source checked in `FJ48`.

### Next action

Begin `FJ48`, `RB-005` FJCw application audit.
