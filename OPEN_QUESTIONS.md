...
Open inside project; generated by `FJ72` and selected for `FJ73`.
...
`FJ73` should compare existing foundational blockers and select at most one
for bounded action. Possible blockers include formulation conventions,
fixed-point conventions, inheritance-source tracing, obstruction-category
notation, transfer-category models, or other recorded open questions. A
blocker is eligible only if the module can name the exact open-question ID,
repository anchor, project object changed, success criterion, failure
