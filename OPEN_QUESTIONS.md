...
Continue with `OQ-072`/`FJ51`, selecting a bounded source or candidate for
`RB-006` without using virtual-special finite-index shortcuts.
...
pauses `RB-005`. Continue with `OQ-072`/`FJ51`.
...
Resolved by `FJ50`. Continue with `OQ-072`/`FJ51`.

## OQ-072. Which source can make RB-006 candidate-ready?

### Status

Open inside project; generated by `FJ50` and selected for `FJ51`.
...
`reflections/strategic_audit_after_cycle_002.md`, and any future
one-relator cubulation or CAT(0)-recognition source selected in `FJ51`.

### Next action

Begin `FJ51`, `RB-006` compact-special/CAT(0)-looking source-selection and
attack-packet module.
