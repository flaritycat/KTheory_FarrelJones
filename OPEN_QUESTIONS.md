...
after `FJ49`. Continue with `OQ-071`/`FJ50`, deciding whether to pause
`RB-005` and pivot.
...
finite-extension bridge. Continue with `OQ-071`/`FJ50`.
...
`FJ49` also blocks the direct CAT(0) finite-extension shortcut. Continue with
`OQ-071`/`FJ50`.
...
a bridge. Continue with `OQ-071`/`FJ50`.
...
Resolved by `FJ49`. Continue with `OQ-071`/`FJ50`, an `RB-005` route-delta
checkpoint and pivot decision.

## OQ-071. Should RB-005 be paused after the finite-index bridge failures?

### Status

Open inside project; generated by `FJ49` and selected for `FJ50`.
...
Begin `FJ50`, `RB-005` route-delta checkpoint and pivot decision.
