...
`OQ-082` and selects `OQ-083` / `FJ61`.
...
Resolved by `reflections/cycle_003_reflection.md`. Continue with `OQ-083`
and `FJ61`.
...
Open inside project; generated by `reflections/cycle_003_reflection.md` and
selected for `FJ61`.
...
`FJ61` must decide what data is required before `T-001` can continue as an
active proof target. It should specify candidate data, route-output
requirements, formulation-safety checks, prior-art comparison fields, and
stop conditions. It may recommend continuing candidate intake, requiring a
specific bridge or computation, pausing `T-001`, or comparing target pivots.
