...
First-pass inventory completed by `FJ69`. Continue only through a bounded
Artin packet such as `FJ70`, not through a broad source survey.
...
Open inside project; sharpened by `FJ69` and selected for `FJ70`.
...
already covered by `FJ18`. The next step is a candidate filter, not another
summary of Wu's source.
...
Open inside project; generated by `FJ69` and selected for `FJ70`.
...
`FJ70` should run a candidate filter, not a broad source survey.
