...
Open inside project; generated by `FJ71` and selected for `FJ72`.
...
The next module should reapply the `FJ67` target-pivot criteria after the
Artin pause. Possible inputs include dormant `T-001`, paused Artin groups,
automatic / biautomatic groups, Thompson-type groups, foundational cleanup,
or no active target, but only if the comparison records source-readiness,
candidate-level object, Farrell--Jones relevance, formulation-safety flags,
known-route overlap, prior-art risk, expected output, stop condition, and
explicit decision.
