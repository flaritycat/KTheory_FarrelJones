...
until `FJ47` selects or verifies a coefficient K-theory finite-index bridge
under the `FJ02` convention.
...
subtraction until `FJ47` selects or verifies a version-compatible
finite-index bridge under the `FJ02` convention.
...
Resolved by `FJ45`, with source-convention policy supplied by `FJ02`.
Continue with `FJ47`, coefficient finite-index bridge source selection.
...
Resolved by `FJ46`; the `FJ02` source-convention interruption is now
complete. Continue with `OQ-068`/`FJ47`.

## OQ-068. Which source verifies a coefficient finite-index overgroup bridge under FJ02?

### Status

Open inside project; generated by `FJ02` and selected for `FJ47`.
...
The next `RB-005` task is therefore narrow: select or verify a source that
gives finite-index overgroup inheritance in the coefficient K-theory
formulation adopted by `FJ02`, or record that no such bridge is currently
...
`ledgers/inheritance_properties.md`, `ledgers/source_status.md`,
`modules/cycle_001/FJ02_additive_categories.md`, and any source checked in
`FJ47`.

### Next action

Begin `FJ47`, coefficient finite-index bridge source selection.
