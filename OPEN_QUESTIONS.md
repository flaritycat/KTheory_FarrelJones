...
payload is recorded. Continue through `OQ-081`/`FJ60`, which must select one
source payload or close the lane as inactive until a source is named.
...
Resolved by `FJ59`. Continue with `OQ-081`/`FJ60`.

## OQ-081. Which exact source supplies a weaker K0 / Cohen--Lyndon payload relevant to T-001?

### Status

Open inside project; generated by `FJ59` and selected for `FJ60`.
...
`ledgers/t001_candidate_inventory.md`, and `OBL-T001-006`.
