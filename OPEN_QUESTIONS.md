...
Open inside project; generated by `FJ64` and selected for `FJ65`.
...
`FJ65` must check whether this no-candidate state still leaves any exact
prior-art blocker object, target-pause recommendation, or target-pivot
comparison to record. It must not begin external source verification or a
proof attempt unless that action changes a project object.
