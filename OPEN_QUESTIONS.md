...
continue with `FJ28`, the residual ledger.
...
subtractions; continue with `FJ28`, the residual ledger.
...
inheritance-route subtraction. Continue with `FJ28`, the residual ledger.
...
First-pass resolved by `FJ27`. Continue with `FJ28`, the residual ledger after
named and inheritance route subtractions.
...
routes and version-aware inheritance routes. The next task is to assemble a
residual ledger describing what has not yet been removed by the project's
recorded bridges.
...
questions generated by those modules, and the `T-001` target-selection
criteria from `FJ20`.
...
Defer until `FJ28` has assembled the residual ledger.
