...
\(\pi(w)>2\) use as hyperbolic overlap. Continue with `OQ-075`/`FJ54`.
...
and `FJ53` resolved `OQ-074`; continue with `OQ-075`/`FJ54`.
...
Resolved by `FJ53`. Continue with `OQ-075`/`FJ54`.

## OQ-075. Which attack packet follows the closed RB-006 path?

### Status

Open inside project; generated by `FJ53` and selected for `FJ54`.
...
Begin `FJ54`, a residual-bucket checkpoint after the closed Louder--Wilton
`RB-006` path.
