...
Open inside project; selected next by `FJ46`.
...
Use module `FJ02` now. `FJ46` selects this as the next project interruption
before further proof-sensitive `RB-005` work.

## OQ-003. Which model of nonconnective algebraic K-theory should be used?

### Status

Open inside project; selected as part of the `FJ02` interruption by `FJ46`.
...
Resolve during the `FJ02` interruption selected by `FJ46`, or explicitly
defer with a source-status reason.
...
Resolve during the `FJ02` interruption selected by `FJ46` before using
`FJCw`, `FICwF`, or finite-wreath-product formulations as proof-level
internal hypotheses.
...
`FJ46` selects the `FJ02` source-convention interruption before further
coefficient/CAT(0) route use.
...
subtraction until the `FJ02` source-convention interruption selected by
`FJ46` is completed and a version-compatible finite-index bridge is
source-verified.
...
Resolved by `FJ46`. Begin the `FJ02` additive-category/source-convention
interruption.
