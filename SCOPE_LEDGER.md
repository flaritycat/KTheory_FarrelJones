...
71. Which `RB-005` cases have source-verified `FJCw` finite-index subgroup input, so that the `FJ47` bridge can be applied? Tracked as `OQ-069`; generated by `FJ47` and selected for `FJ48`.
...
finite-index subgroup input so that the `FJ47` bridge can be applied.
