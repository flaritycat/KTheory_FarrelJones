...
73. Should `RB-005` be paused after the failed `FJCw` and direct CAT(0) finite-extension application checks, and which attack packet should replace it? Tracked as `OQ-071`; generated by `FJ49` and selected for `FJ50`.
...
Begin `FJ50`, an `RB-005` route-delta checkpoint and pivot decision after the
failed `FJCw` and direct CAT(0) finite-extension application checks.
