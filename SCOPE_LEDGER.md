...
75. Does Louder--Wilton's negative-immersion source produce an `RB-006` compact-special/CAT(0) route delta, or only hyperbolic-route overlap already governed by `FJ23`? Tracked as `OQ-073`; generated by `FJ51` and selected for `FJ52`.
...
Begin `FJ52`, a Louder--Wilton negative-immersion boundary check for
`RB-006`.
