...
92. Which candidate target, if any, should `FJ68` select after applying the `FJ67` target-pivot criteria? Tracked as `OQ-090`; generated by `FJ67` and selected for `FJ68`.
...
After `FJ66` and `FJ67`, `T-001` is dormant rather than active. It remains
the archived first serious target, but no active proof-target sequence is
currently selected. A later module may reactivate `T-001` only through
`OBL-T001-013`; otherwise `FJ68` must compare possible target pivots under
`OBL-PIVOT-001`.
