...
130. Which accepted payload, if any, should exit the cycle-006 payload-gated pause and instantiate the next numbered module? Tracked as `OQ-126`; partially resolved by `FJ101` for queue row `001`, by `FJ102` for queue row `002`, by `FJ103` for queue row `003`, by `FJ104` for queue row `004`, by `FJ105` for queue row `005`, by `FJ106` for queue row `006`, by `FJ107` for queue row `007`, and by `FJ108` for queue row `008` only.
...
- continue-payload queue and FJ101--FJ108 intake: `FJ101` consumes only row
  ...
  `FJ108` consumes only row `008` of `ledgers/payload_execution_queue.md`,
  adding `CAND-T001-C6-001` through `CAND-T001-C6-008` as
  ...
  `OBL-C6-009`, `OQ-133` / `OBL-C6-010`, and `OQ-134` / `OBL-C6-011`, and
...
`ledgers/payload_execution_queue.md` rows `001`--`008` have been completed by
`FJ101`--`FJ108`; the next mathematical move requires either the exact
