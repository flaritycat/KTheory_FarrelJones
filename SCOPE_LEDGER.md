...
93. Can `FJ69` produce an Artin subclass-gap inventory from the existing `FJ18` rows without turning into a broad source-summary module? Tracked as `OQ-091`; generated by `FJ68` and selected for `FJ69`.
...
Current active target packet: `A-001`, Artin subclass-gap inventory after
`FJ18`.
...
The current Artin packet is bounded: it asks for a subclass-gap inventory
after `FJ18`, not a proof or claim of Farrell--Jones for all Artin groups.
