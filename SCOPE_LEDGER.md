...
26. Is there a global K-theoretic Farrell--Jones theorem for all torsion-free one-relator groups in a weaker formulation than Full Farrell--Jones? Tracked as `OQ-024`; flagged by `FJ54` for later lower-priority update, not resolved.
...
78. Can Linton-style hierarchy / primitive-extension data be converted into an FJ route, a candidate family, a bridge lemma, or a documented obstruction when paired with kernel-control testing? Tracked as `OQ-076`; generated by `FJ54` and selected for `FJ55`.
