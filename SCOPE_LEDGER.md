...
130. Which accepted payload, if any, should exit the cycle-006 payload-gated pause and instantiate the next numbered module? Tracked as `OQ-126`; partially resolved by `FJ101` for queue row `001`, by `FJ102` for queue row `002`, by `FJ103` for queue row `003`, by `FJ104` for queue row `004`, by `FJ105` for queue row `005`, by `FJ106` for queue row `006`, by `FJ107` for queue row `007`, by `FJ108` for queue row `008`, by `FJ109` for queue row `009`, and by `FJ110` for queue row `010` only.
...
- continue-payload queue and FJ101--FJ110 intake: `FJ101` consumes only row
  ...
  `FJ110` consumes only row `010` of `ledgers/payload_execution_queue.md`,
  adding `CAND-T001-C6-001` through `CAND-T001-C6-010` as
  ...
  `OQ-135` / `OBL-C6-012`, and `OQ-136` / `OBL-C6-013`, and making no
...
`ledgers/payload_execution_queue.md` rows `001`--`010` have been completed by
`FJ101`--`FJ110`; the next mathematical move requires either the exact
