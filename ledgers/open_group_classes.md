...
| Torsion-free one-relator groups | selected first serious target `T-001`; global class remains unresolved inside project; `FJ19` records Lueck's open-status warning for Full Farrell--Jones and verified conditional routes; `FJ22` records the adopted one-relator hierarchy vocabulary; `FJ23` subtracts the hyperbolic route once a source-verified hyperbolicity bridge is available | finite-dimensional CAT(0), virtually solvable, hyperbolic-by-cyclic/free-by-cyclic, and other HNN/hierarchy cases not already removed by the hyperbolic route | FJ19, FJ20+ | Continue with `FJ24`: subtract the CAT(0) route and source-check any virtual-special bridge before use. |
