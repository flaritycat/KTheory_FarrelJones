...
| Torsion-free one-relator groups | selected first serious target `T-001`; global class remains unresolved inside project; `FJ19` records Lueck's open-status warning for Full Farrell--Jones and verified conditional routes | hyperbolic, finite-dimensional CAT(0), virtually solvable, hyperbolic-by-cyclic/free-by-cyclic, and other HNN/hierarchy cases | FJ19, FJ20+ | Begin with `FJ21`: choose the one-relator structure source toolkit, then build the residual gap map. |
