...
| Torsion-free one-relator groups | selected first serious target `T-001`; global class remains unresolved inside project; `FJ19` records Lueck's open-status warning for Full Farrell--Jones and verified conditional routes; `FJ22` records the adopted one-relator hierarchy vocabulary; `FJ23` subtracts the hyperbolic route; `FJ24` subtracts the finite-dimensional CAT(0) route and compact special bridge cases; `FJ25` subtracts the virtually solvable route only when a virtual-solvability bridge is recorded; `FJ26` subtracts the hyperbolic-by-cyclic/free-by-cyclic route only when a mapping-torus or finite-rank free-by-cyclic bridge is recorded | source-verified inheritance routes, finite-index handling, kernel-recognition questions, and other HNN/hierarchy cases not already removed by the named routes | FJ19, FJ20+ | Continue with `FJ27`: subtract source-verified inheritance routes while preserving formulation/version flags. |
