...
| Torsion-free one-relator groups | selected first serious target `T-001`; global class remains unresolved inside project; `FJ19` records Lueck's open-status warning for Full Farrell--Jones and verified conditional routes; `FJ22` records the adopted one-relator hierarchy vocabulary; `FJ23` subtracts the hyperbolic route; `FJ24` subtracts the finite-dimensional CAT(0) route and compact special bridge cases | virtually solvable, hyperbolic-by-cyclic/free-by-cyclic, and other HNN/hierarchy cases not already removed by the hyperbolic or CAT(0) routes | FJ19, FJ20+ | Continue with `FJ25`: subtract the virtually solvable route without treating local indicability as virtual solvability. |
