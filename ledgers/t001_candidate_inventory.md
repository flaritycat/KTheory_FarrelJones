...
`CAND-T001-005`, and by `FJ92` with the Brown/BNS kernel-control computation
for `CAND-T001-005`.
...
| `CAND-T001-005` | \(G_{FJ90}=\langle a,b\mid a b a^{-1} b^2 a b^{-3}\rangle\) | no, first-pass internal abelianization check in `FJ90` | source-verified at first pass in `FJ91` via Putman's statement of the Karrass--Magnus--Solitar torsion theorem plus the `FJ90` non-proper-power check | not used | unknown from current project records | unknown from current project records | Brown-positive in `FJ92`; \([\chi]\) and \([-\chi]\) pass the zero-on-one-generator maximum-count test | yes, \(\chi(a)=0,\chi(b)=1\) | finitely generated by Brown; not identified as finite-rank free | possible `RB-004` / `RB-008` route only after finite-rank freeness, another route bridge, or a prior-art blocker is checked | no route formulation selected; keep full FJ, coefficient FJC, `FJCw`, and weaker \(K_0\) labels separate | unknown / unchecked | `OBL-T001-020`: identify finite-rank free kernel, supply known-route overlap, or record prior-art blocker before promotion | stop if treated as routed, finite-rank free-by-cyclic, or novel from Brown finite generation alone |
...
| `OBL-T001-020` | active requirement | `FJ92` | Brown finite generation is now recorded for `CAND-T001-005`, but this is not yet a finite-rank free-kernel bridge and not an `FJ26` route. | Record a new accepted payload for finite-rank free-kernel identification, known-route / prior-art blocker audit, formulation comparison, or another checked route bridge. | Stop if Brown-positive finite generation is promoted to a Farrell--Jones route without the missing kernel-type or route/prior-art data. |
...
No active `T-001` proof-target update is scheduled after `FJ92` unless a
...
payload protocol. No `FJ93` module is selected without a new accepted payload.
