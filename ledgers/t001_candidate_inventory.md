...
| `OBL-T001-022` | active requirement | `FJ94` | `CAND-T001-005` has passed intake, torsion-free, Brown finite-generation, known-route/prior-art, and FJ83-hypothesis audits. It remains route-blocked and not FJ83-eligible from current data. | A future payload must supply a finite-rank free-kernel identification, source-verified route bridge, new FJ83 hypothesis package, formulation comparison, prior-art object, or governance branch decision. | Stop if the row is treated as solved, routed, FJ83-eligible, or closed without the corresponding recorded payload. |
...
it. Future route or branch use is governed by `OBL-T001-022`.

## Next update

No active `T-001` proof-target update is scheduled after `FJ94` unless a
future accepted payload supplies finite-rank free-kernel identification,
source-verified route bridge, new FJ83 weaker \(K_0\) hypothesis data,
formulation comparison, branch/pause decision, or another prior-art object
satisfying the payload protocol. No `FJ95` module is selected without a new
accepted payload.
