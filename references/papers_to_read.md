...
## Priority 1: foundational statement and assembly maps
...
2. Bestvina, M.; Fujiwara, K.; Wigglesworth, D. *The Farrell-Jones conjecture for hyperbolic-by-cyclic groups*. Status: active reference for `FJ19`, `FJ26`, `FJ33`, `FJ36`, and `FJ37`; main theorem, coefficient convention, and extension theorem checked at first-pass level. `FJ33` uses the existing `FJ26` route for \(G_{2,3}\), `FJ36` uses it for the \(G_{p,q}\)-family after the finite-rank free-by-cyclic bridge is verified, and `FJ37` uses it only through the existing `FJ26` route ledger.
...
6. Linton, M. *Hyperbolic one-relator groups*. Status: active bridge source for `FJ21` and `FJ22`; use for orientation while `One-relator hierarchies` controls exact vocabulary.
...
13. Serre, J.-P. *Trees*, chapter "Trees and Amalgams". Status: active reference for `FJ36`; Springer bibliographic and chapter metadata checked in `FJ35`; `FJ36` uses Serre as the canonical Bass--Serre source for the \(K_{p,q}\) freeness bridge.
14. Carrasco, M.; Mackay, J. M. *Conformal dimension of hyperbolic groups that split over elementary subgroups*. Status: active auxiliary reference for `FJ36`; used for the graph-of-groups/Bass--Serre stabilizer formulation, citing Serre.
15. Chiswell, I.; Mueller, T. *A universal construction for groups acting freely on real trees*. Status: active auxiliary reference for `FJ36`; used only for the free-action bridge.
16. Margalit, D. "Office hour three: Groups acting on trees." Status: active auxiliary reference for `FJ36`; used for the explicit theorem that a group acting freely on a tree is free.

Next one-relator reading task: for `FJ38`, source-select for `RB-004` cases
outside Brown's checked two-generator criterion. Begin with the existing
Bieri--Neumann--Strebel and one-relator structure source queues, and add any
new source only after bibliographic and hypothesis checks.
...
- hypotheses;
- theorem/section number;
- whether the source was fully checked, partially checked, or only located;
